-- A. Tạo DATABASE
DROP DATABASE IF EXISTS taxi_management; -- Xóa db nếu đã tồn tại
CREATE DATABASE taxi_management CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE taxi_management;

-- A1. Bảng Tài xế (ĐÃ SỬA KIỂU CỘT)
CREATE TABLE TAI_XE (
    ...
    trang_thai VARCHAR(50) DEFAULT 'DANG_LAM_VIEC'
);
...
CREATE TABLE XE (
    ...
    nam_sx VARCHAR(50),
    -- SỬA TỪ ENUM SANG VARCHAR
    trang_thai_xe VARCHAR(50) DEFAULT 'SAN_SANG',
    ma_loai VARCHAR(50),
...
-- A8. Bảng Phân Công Xe (Bảng mới)
CREATE TABLE PHAN_CONG_XE (
    ma_tai_xe VARCHAR(50),
    ma_xe VARCHAR(50),
    thoi_gian_bat_dau DATETIME NOT NULL, -- <-- Phải thêm cột này vào khóa
    thoi_gian_ket_thuc DATETIME NULL,
    
    FOREIGN KEY (ma_tai_xe) REFERENCES TAI_XE(ma_tai_xe),
    FOREIGN KEY (ma_xe) REFERENCES XE(ma_xe),
    
    -- Gộp 3 cột làm khóa chính
    PRIMARY KEY (ma_tai_xe, ma_xe, thoi_gian_bat_dau) 
);
...
-- B1. FUNCTION 1: Hàm tính cước phí
DELIMITER //
...
-- B2. FUNCTION 2: Hàm xác định giờ cao điểm
DELIMITER $$
...
BEGIN
    ...
    IF current_day_of_week > 1 AND current_day_of_week < 7 THEN
        IF (current_hour >= 7 AND current_hour < 8)
           OR
           (current_hour >= 17 AND current_hour < 18)
           OR
           (current_hour >= 22 AND current_hour <= 24)
           OR
           (current_hour >= 0 AND current_hour <= 5)
        THEN
            RETURN TRUE;
        END IF;
    END IF;
...
-- DROP TRIGGER trg_update_vehicle_status
CREATE TRIGGER trg_update_vehicle_status
AFTER INSERT ON CHUYEN_DI
FOR EACH ROW
BEGIN
    UPDATE XE SET trang_thai_xe = 'DANG_CHAY' -- Sửa giá trị ENUM
...
BEGIN
    ...
    FROM xe
    WHERE ma_xe = NEW.ma_xe; 
    IF verhicle_status != 'SAN_SANG' -- Sửa giá trị ENUM
    THEN
...
BEGIN
	UPDATE xe
    SET trang_thai_xe = 'BAO_TRI' -- Sửa giá trị ENUM
    WHERE ma_xe=NEW.ma_xe;
END$$
DELIMITER ;
...
-- B7. STORED PROCEDURE 2: Thống kê chuyến đi theo giờ
DELIMITER $$
...
-- B8. STORED PROCEDURE 3: Thống kê tổng chi phí bảo trì xe theo từng tháng
DELIMITER $$
...
-- B9. STORED PROCEDURE 4: Hoàn tất chuyến đi
DELIMITER $$
-- DROP PROCEDURE IF EXISTS SP_HoanTatChuyenDi;
CREATE PROCEDURE SP_HoanTatChuyenDi(
    IN p_ma_chuyen VARCHAR(50),
    IN p_so_km_di DOUBLE
)
BEGIN
    ...
    -- Lỗi trong SP gốc: fn_KiemTraGioCaoDiem không tồn tại, phải là fn_XacDinhGioCaoDiem
    SET v_is_peak = fn_XacDinhGioCaoDiem(); 
...
('TX001',   'Nguyễn Văn An',    'GPLX001', '2002-05-15', '0983456789', 'DANG_LAM_VIEC'),
('TX002',   'Trần Thị Bích',    'GPLX002', '2000-11-28', '0912345678', 'DANG_LAM_VIEC'),
('TX003',   'Lê Quang Cường',   'GPLX003', '2004-01-03', '0907890123', 'DANG_LAM_VIEC'),
('TX004',   'Phạm Minh Dũng',   'GPLX004', '2001-08-19', '0961234567', 'NGHI_VIEC'),
('TX005',   'Hoàng Thu Giang',  'GPLX005', '2003-04-10', '0978901234', 'DANG_LAM_VIEC'),
('TX006',   'Đỗ Trọng Hiếu',    'GPLX006', '2000-12-07', '0945678901', 'DANG_LAM_VIEC'),
('TX007',   'Bùi Thanh Hương',  'GPLX007', '2002-06-22', '0923456789', 'RANH'),
('TX008',   'Vũ Tuấn Khanh',    'GPLX008', '2004-03-01', '0938901234', 'NGHI_VIEC'),
('TX009',   'Đặng Việt Lâm',    'GPLX009', '2001-09-14', '0881122334', 'RANH'),
('TX010',   'Lương Hồng Sơn',   'GPLX010', '2003-04-23', '0963344556', 'DANG_LAM_VIEC');
...
-- C3. XE
INSERT INTO XE (ma_xe, bien_so_xe, mau_xe, nam_sx, trang_thai_xe, ma_loai) VALUES
('XE001', '51A-00001', 'Trắng',     '2015', 'SAN_SANG',  'LX004'),
('XE002', '51A-00002', 'Xám',       '2023', 'SAN_SANG',  'LX003'),
('XE003', '51A-00003', 'Đỏ',        '2017', 'SAN_SANG',  'LX002'),
('XE004', '51A-00004', 'Xanh lam',  '2017', 'SAN_SANG',  'LX001'),
('XE005', '51A-00005', 'Đen',       '2019', 'SAN_SANG',  'LX001'),
('XE006', '51A-00006', 'Hồng',      '2024', 'SAN_SANG',  'LX002'),
('XE007', '51A-00007', 'Vàng',      '2016', 'BAO_TRI' ,  'LX003'),
('XE008', '51A-00008', 'Vàng',      '2015', 'SAN_SANG',  'LX004'),
('XE009', '51A-00009', 'Trắng',     '2021', 'SAN_SANG',  'LX004'),
('XE010', '51A-00010', 'Đen',       '2022', 'SAN_SANG',  'LX003'),
('XE011', '51A-00011', 'Đen',       '2022', 'BAO_TRI',   'LX002'),
('XE012', '51A-00012', 'Đỏ',        '2019', 'BAO_TRI',   'LX001'),
('XE013', '51A-00013', 'Xanh lam',  '2025', 'BAO_TRI',   'LX001'),
('XE014', '51A-00014', 'Xanh lục',  '2017', 'DANG_CHAY', 'LX002'),
('XE015', '51A-00015', 'Xanh lục',  '2019', 'DANG_CHAY', 'LX003');
...
('KH0011', 'Tôn Minh Lộc',       '0379870898'),
...
('CD008', 'Siêu thị Cần Thơ', 'Đường 30/4, Cần Thơ',
    '2025-05-05 08:00:00', '2025-05-05 08:18:00', 6.50, 105000, 'XE010', 'KH002'),
('CD005', 'Vincom Xuân Khánh, Cần Thơ', 'Bến xe khách Cần Thơ',
    '2025-02-02 11:20:00', '2025-02-02 11:45:00', 7.90, 125000, 'XE010', 'KH002'),
('CD011', 'Phường 3, TP. Vĩnh Long', 'Phường 4, TP. Vĩnh Long',
    '2025-08-08 21:00:00', '2025-08-08 21:10:00', 3.50, 65000,  'XE011', 'KH018'),
('CD012', 'Cầu Rạch Súc, Ô Môn, Cần Thơ', 'QL91, Thốt Nốt, Cần Thơ', 
    '2025-09-09 06:45:00', '2025-09-09 07:30:00', 21.3, 340000, 'XE010', 'KH010'),
('CD013', 'Bến xe Vĩnh Long', 'Công viên Sông Hậu, Cần Thơ', 
    '2025-10-10 12:00:00', '2025-10-10 13:10:00', 45.0, 750000, 'XE011', 'KH016'),
('CD014', 'Huyện Tam Bình, Vĩnh Long', 'Huyện Long Hồ, Vĩnh Long', 
    '2025-11-11 15:30:00', '2025-11-11 16:15:00', 18.2, 290000, 'XE010', 'KH007');
...
-- C8. DU LIEU PHAN CONG XE (MỤC MỚI)
-- (Khóa chính là: ma_tai_xe, ma_xe, thoi_gian_bat_dau)
-- Giả sử hôm nay là 2025-11-11

-- Các ca đã hoàn thành
INSERT INTO PHAN_CONG_XE (ma_tai_xe, ma_xe, thoi_gian_bat_dau, thoi_gian_ket_thuc) VALUES
('TX007', 'XE010', '2025-11-01 06:00:00', '2025-11-01 18:00:00'),
('TX009', 'XE005', '2025-11-02 08:00:00', '2025-11-02 20:00:00'),
('TX007', 'XE010', '2025-11-03 06:00:00', '2025-11-03 18:00:00');

-- Một ca đang chạy (ket_thuc = NULL)
-- Tài xế TX009 (RANH) lái xe XE006 (SAN_SANG, vì TX008 nghỉ)
INSERT INTO PHAN_CONG_XE (ma_tai_xe, ma_xe, thoi_gian_bat_dau, thoi_gian_ket_thuc) VALUES
('TX009', 'XE006', '2025-11-11 06:00:00', NULL);